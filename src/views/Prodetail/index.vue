  ...
  <div class="prodetail">
    ...
    <div class="comment">
      <div class="comment-title">
        <div class="left">商品评价 ({{ list.length }}条)</div>
    ...
    <div class="footer">
      ...
      <div class="btn-add" @click="add('cart')">加入购物车</div>
      <div class="btn-buy" @click="add('buy')">立刻购买</div>
    </div>
...
<script>
import { GetCartTotal, addCartList } from '@/api/cart';
import { GetProdetailListAPI, GetViews } from '@/api/pordetail'
import defaultIMG from "@/assets/leina.avif";
import countBox from '@/components/countBox.vue'
import From from '@/mixins/From';
export default {
  mixins: [From],
  components: {
    countBox
  ...
  methods: {
    ...
    //获取详情页数据
    async GetProdetailListData() {
      const res = await GetProdetailListAPI(this.goodsId)
      this.images = res.data.detail.goods_images;
      this.detail = res.data.detail
    },
    //获取评论
    async GetViewsData() {
      const res = await GetViews(this.goodsId)
